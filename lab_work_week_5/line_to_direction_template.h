...
template<typename T>
void list_two_direcktion<T>::Show() {
...
template<typename T>
void list_two_direcktion<T>::Add_to_head(T data) {
	if (this->elem == nullptr) {
...
template<typename T>
void list_two_direcktion<T>::Add_to_tail(T data) {
	if (this->elem == nullptr) {
		this->elem = new Element(data);
	}
	else
...
template<typename T>
void list_two_direcktion<T>::Add_to_position(int position, T data) {
	if (position == 0) {
...
template<typename T>
void list_two_direcktion<T>::Delete_from_head() {
...
template<typename T>
void list_two_direcktion<T>::Delete_from_tail() {
	if (this->elem->next == nullptr) {
		Element* temp = this->elem;
		delete temp;
		temp = nullptr;
...
template<typename T>
void list_two_direcktion<T>::Delete_all() {
...
template<typename T>
void list_two_direcktion<T>::Delete_from_position(int position) {
	if (position == 0) {
		this->Delete_from_head();
	}
	else
...
template<typename T>
int list_two_direcktion<T>::Search(T key) {
	Element* temp = this->elem;
	int posis{ 0 };
	while (temp!=nullptr)
	{
...
template<typename T>
int list_two_direcktion<T>::Search_and_replace(int position, T key, T keyforreplace) {
...
template<typename T>
void list_two_direcktion<T>::Direction_swap() {
	Element* temp = this->elem;
	Element* newlist;
	while (temp->next!=nullptr)
	{
